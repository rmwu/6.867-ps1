...
\begin{figure*}
   ...
   \caption{Batch gradient descent (left) and stochastic gradient descent (right) on the sum of squares error for linear regression on real vectors in 10 dimensions. We see that SGD requires more iterations to converge and fluctuates more, in both objective function and gradient, than batch gradient descent.} % on which objective function?
   \label{fig:1.2}
\end{figure*}
